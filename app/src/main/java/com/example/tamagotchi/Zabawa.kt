...
class Zabawa(var context: Context, var level: Int = 100) {

    private var pasekZabawyListener: PasekZabawyListener? = null
    ...
    interface PasekZabawyListener {
        fun onPasekZabawyChange(bitmapResource: Int)
    }

    fun setPasekZabawyListener(listener: PasekZabawyListener) {
        this.pasekZabawyListener = listener
    }
    ...
    private fun zmniejszPasek() {
        if (level > 0) {
            level -= 2
            if (level % 10 == 0) {
                pasekZabawyListener?.onPasekZabawyChange(getObrazDlaPoziomu(level))

            }

            if (level < 50 && czy_noti) {
                showNoti(context.applicationContext, "Tamagotchi jest smutny :(", "Pobaw się ze mną!")
                czy_noti = false
            }
    ...
    fun zwiekszZabawe(amount: Int) {
        if (level < 100) {
            level = minOf(100, level + amount) // Zwiększ poziom zabawy
        }
        if (level % 10 == 0) {
    ...
    // Metoda do mapowania poziomu na odpowiedni obraz paska zabawy
